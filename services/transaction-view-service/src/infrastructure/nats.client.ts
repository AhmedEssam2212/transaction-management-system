  ...
  private async setupStreams(): Promise<void> {
    ...
    try {
      ...
      const transactionStreamConfig: Partial<StreamConfig> = {
        ...
        subjects: [
          NATS_SUBJECTS.TRANSACTION_CREATED,
          NATS_SUBJECTS.TRANSACTION_UPDATED,
          NATS_SUBJECTS.TRANSACTION_DELETED,
          NATS_SUBJECTS.TRANSACTION_ROLLBACK,
        ],
        max_age: 30 * 24 * 60 * 60 * 1000000000, // 30 days in nanoseconds
      };

      try {
        await this.jsm.streams.info(NATS_STREAMS.TRANSACTIONS);
        console.log(`Stream ${NATS_STREAMS.TRANSACTIONS} already exists`);
      } catch {
        await this.jsm.streams.add(transactionStreamConfig);
        console.log(`Created stream ${NATS_STREAMS.TRANSACTIONS}`);
      }
    } catch (error) {
